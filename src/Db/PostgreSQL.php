...
class PostgreSQL extends Database {
	...
	public function __construct() {
		$host = Env::string('POSTGRESQL_HOST', '127.0.0.1');
		$port = Env::int('POSTGRESQL_PORT', 5432);
		$db = Env::string('POSTGRESQL_DB', 'snd_pgsql');
		$user = Env::string('POSTGRESQL_USER', 'postgres');
		$password = Env::string('POSTGRESQL_PASSWORD');
		$this->connect(
	...
	public function limit(int $rows = 0, int $offset = 0): static {
		$this->pieces['limit'] = "LIMIT $rows";
		if ($offset > 0) {
			$this->pieces['limit'] .= " OFFSET $offset";
		}
		return $this;
	}
